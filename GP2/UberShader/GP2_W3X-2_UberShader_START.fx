...
float3 CalculateSpecularBlinn(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float specStrength = pow(saturate(dot(-normal, halfVector)), gShininess);
	
	float intensity = 1.f;
	if(gUseTextureSpecularIntensity)
	{
		intensity = gTextureSpecularIntensity.Sample(gTextureSampler, texCoord);
	}

	
	return gColorSpecular * specStrength * intensity;
}
...
float3 CalculateSpecularPhong(float3 viewDirection, float3 normal, float2 texCoord)
{
	...
	float specStrength = pow(saturate(dot(reflectVector, -viewDirection)), gShininess);
...
float3 CalculateNormal(float3 tangent, float3 normal, float2 texCoord)
{
	...
	float3 finalNormal = normalize(lerp(normal, transformedNormal, gNormalMapStrength));
...
float3 CalculateDiffuse(float3 normal, float2 texCoord)
{
	...
	float3 diffuseFactor = pow((dot(gLightDirection, -normal) * 0.5f + 0.5f), 2);
	
		
	return diffColor * diffuseFactor;
}
...
float3 CalculateFresnelFalloff(float3 normal, float3 viewDirection, float3 environmentColor)
{
	...
	float fresnelFactor =  pow(1 - saturate(abs(dot(normal, viewDirection))) , gFresnelPower) * gFresnelMultiplier;
	float fresnelMask = pow(1 - saturate(dot(float3(0.f, -1.f, 0.f), normal)), gFresnelHardness);

	if(gUseFresnelFalloff)
	{
		return fresnelColor * fresnelFactor * fresnelMask;
	}
	else
	{
		return float3(0,0,0);
...
float3 CalculateEnvironment(float3 viewDirection, float3 normal)
{
	...
	float3 refractionVector = refract(viewDirection, -normal, gRefractionIndex);
	
	
	float3 reflectionColor = gCubeEnvironment.Sample(gTextureSampler, reflectionVector) * gReflectionStrength;
	float3 refractionColor = gCubeEnvironment.Sample(gTextureSampler, refractionVector) * gRefractionStrength;
