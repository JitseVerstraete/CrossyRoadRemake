...
MeshFilter* MeshFilterLoader::LoadContent(const ContentLoadInfo& loadInfo)
{
	...
	for (;;)
	{
		...
		{
			uint16_t clipCount{ pReader->Read<uint16_t>() };
			for (int c = 0; c < clipCount; c++) //for every clip
			{
				AnimationClip clip{};
				...
				uint16_t keyCount = pReader->Read<uint16_t>();
				for (int k = 0; k < keyCount; k++) //for every key
				{
					AnimationKey key{};
					key.tick = pReader->Read<float>();

					uint16_t boneCount = pReader->Read<uint16_t>();
					for (int b = 0; b < boneCount; ++b) //for every bone
					{
						XMFLOAT4X4 boneTransform = pReader->Read<XMFLOAT4X4>();
						key.boneTransforms.emplace_back(boneTransform);
					}
