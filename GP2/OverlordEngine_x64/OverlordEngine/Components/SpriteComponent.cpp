...
void SpriteComponent::Draw(const SceneContext& sceneContext)
{
	...
	XMFLOAT2 pos = { transform->GetPosition().x, transform->GetPosition().y };


	SpriteRenderer::Get()->AppendSprite(m_pTexture, pos, m_Color, m_Pivot, XMFLOAT2{ transform->GetScale().x, transform->GetScale().y }, MathHelper::QuaternionToEuler(transform->GetRotation()).z, transform->GetPosition().z);
