...
void ModelComponent::ShadowMapDraw(const SceneContext& sceneContext)
{
	...
	if (HasAnimator())
	{
		ShadowMapRenderer::Get()->DrawMesh(sceneContext, m_pMeshFilter, GetGameObject()->GetTransform()->GetWorld(), m_pAnimator->GetBoneTransforms());
	}
	else
	{
		ShadowMapRenderer::Get()->DrawMesh(sceneContext, m_pMeshFilter, GetGameObject()->GetTransform()->GetWorld());
	}
}
