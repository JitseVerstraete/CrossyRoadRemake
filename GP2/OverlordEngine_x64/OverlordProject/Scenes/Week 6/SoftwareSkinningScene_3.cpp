...
void SoftwareSkinningScene_3::Initialize()
{
	...
	m_pBone0 = new BoneObject(pMat, boneLength);
	pRoot->AddChild(m_pBone0);
	m_pBone1 = new BoneObject(pMat, boneLength);
...
void SoftwareSkinningScene_3::Update()
{
	...
	XMMATRIX boneTransform0Matrix = XMLoadFloat4x4(&m_pBone0->GetBindPose()) * XMLoadFloat4x4(&m_pBone0->GetTransform()->GetWorld());
	XMMATRIX boneTransform1Matrix = XMLoadFloat4x4(&m_pBone1->GetBindPose()) * XMLoadFloat4x4(&m_pBone1->GetTransform()->GetWorld());


	//transform skinned vertices
	XMVECTOR tranformVector{};
	for (int i{}; i < m_SkinnedVertices.size(); i++)
	{
		XMVECTOR transform0 = XMVector3TransformCoord(XMLoadFloat3(&m_SkinnedVertices[i].originalVertex.Position), boneTransform0Matrix) * m_SkinnedVertices[i].blendWeight0;
		XMVECTOR transform1 = XMVector3TransformCoord(XMLoadFloat3(&m_SkinnedVertices[i].originalVertex.Position), boneTransform1Matrix) * m_SkinnedVertices[i].blendWeight1;
		tranformVector = transform0 + transform1;

		XMFLOAT3 transformedVertexPos{};
		XMStoreFloat3(&transformedVertexPos, tranformVector);
		m_SkinnedVertices[i].transformedVertex.Position = transformedVertexPos;
	}
